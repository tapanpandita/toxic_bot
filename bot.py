...
async def get_bj_joke(ctx):
    async with aiohttp.ClientSession() as session:
        async with session.get('http://api.icndb.com/jokes/random?firstName=Bannanje&lastName=Nayak') as resp:
...
id:32 Entertainment: Cartoon & Animations
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(f'https://opentdb.com/api.php?amount=1&category={category}') as resp:
                response = await resp.json()
                result = response.get("results")[0]
                question = unescape(result.get("question"))
                options = result.get("incorrect_answers") + [result.get("correct_answer")]
                ...
                await ctx.send('"{}" is the correct answer!'.format(unescape(result.get("correct_answer"))))


@bot.command(name='qr', help='Posts a qr code for the url')
async def convert_to_qr_image(ctx, data):
    data = quote(data)
    await ctx.send(f'https://api.qrserver.com/v1/create-qr-code/?size=250X250&data={data}')
